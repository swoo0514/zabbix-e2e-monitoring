...
echo "→ 아이템 조회: key=$KEY"
ITEMID=$(api item.get "$(jq -n --arg k "$KEY" '{output:["itemid","name"],search:{key_:$k}}')" "$TOKEN" | jq -r '.result[0].itemid // empty')
...
if echo "$RES" | jq -e '.result.itemids' >/dev/null 2>&1; then
  echo "✅ 갱신 완료 (itemid=$ITEMID). 이제 Zabbix에서 Execute now."
else
  echo "❌ 업데이트 실패:"; echo "$RES" | jq .
  exit 1
fi
