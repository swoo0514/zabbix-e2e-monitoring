...
- **톤:** 기술 블로그처럼 간결·단정하게 쓴다. 불필요한 사과·과장·AI투 상투어("~해보겠습니다 😊" 남발 등) 지양. 채팅·Notion 문서 공통.
...
## 고도화 진행 상태 (2026-07-06 착수 — 항상 최신 유지)

필수 파이프라인(nginx Web Scenario + midibus 5-Step Browser Item + dependent + trigger + Slack PROBLEM/RESOLVED)은 **완료**. 아래는 그 위에 얹는 **고도화 구현예정 9개**(Notion "확장 전략 카탈로그"의 📌 로드맵)의 **확정 우선순위**. 실작업 로그: `private/업무보고/작업로그_고도화.md`.

**정렬 축:** ① 검증된 5-Step 스크립트를 건드리는 리스크 ② 노력 ③ 임팩트·인사이트. → **검증된 스크립트는 최대한 늦게 손댄다**(1~4는 순수 Zabbix 설정, 리스크 0).

1. ~~**nodata() 헬스체크**~~ ✅ 07-06 완료 (3층 번들: nodata 검증완료 + server health 검증 + 층3 설계. 용량실측: 시나리오 ~26s/회, 평시 폴러 busy ≈4%, 여유 ≈17배 — 당초 40%/1.9배 해석은 1분 샘플창 오독으로 정정됨)
2. ~~**Services/SLA 가용성%**~~ ✅ 07-06 완료 (트리 7개+SLA 99.5%, fault injection 검증 통과. category 아이템 Disabled 발견·복구, securitykey/subuser dependent+trigger 신설)
3. ~~**성능마크 이상치 트리거**~~ ✅ 07-06 완료 (perf 아이템 4종+Warning 트리거 4종. 시나리오 벽시계 26s 확정, 마크≠스텝시간 발견→스텝 벽시계는 #5로)
4. ~~**Zabbix 대시보드**~~ ✅ 07-06 완료 (위젯 8종: Problems/SLA report/Item value×2/perf 이중축 그래프/스텝 상태+Honeycomb/인프라 헬스. + nginx는 "Nginx by HTTP" 템플릿으로 stub_status 수집 개시)
5. ~~서브액션 추적 + 에러 분류~~ ✅ 07-06 완료 (스크립트 +20줄 계측: crumb→err_at / err_class / step_seconds + dependent 7종. 정상·에러 경로 검증 통과. 커밋 6f0ddb1)
6. ~~**셀렉터 폴백 + self-heal**~~ ✅ 07-07 (커밋 615ef14) — 메커니즘·스텝격리·only 파라미터·트리거 별형 재편 구현·검증. **폴백 셀렉터 시딩만 잔여**(실DOM 확보 후 다음 세션). 온디맨드 아이템 `browser.midibus.ondemand`로 부분 실행(FAST 대체). 전체 기록 `private/docs/browser-item-architecture.md` §11 + 노션 셀렉터폴백 페이지
7. Config-as-Code (host/trigger/action API 프로비저닝)
8. Kafka 유실률 비교 실험 *(stretch, Day10)*
9. 교차 브라우저 스파이크 (Chrome↔Firefox) *(stretch, Day10)*

- Playwright 하이브리드는 이 9개 밖의 ⚪설계·추론(헤드라인) — 관측 기반(1~4) 선행 후 별도 트랙.
- **코드 구현은 사용자가 "구현해/진행" 등으로 명시할 때만.** 그 외에는 계획·문서만.

**⏰ 예정된 후속 확인 (Claude가 해당 날짜 이후 세션에서 먼저 상기할 것):**
- **2026-07-09~10:** #3 perf baseline 판정 품질 점검 — Monitoring→Problems 히스토리에 `type:performance` Warning **오탐**이 있었는지 + Latest data에서 perf 4종 수집 지속 확인. 문제없으면 그때 Slack 승격 여부 결정. (avg(7d)는 쌓인 만큼으로 계산되므로 2~3일치면 판정 안정성 확인 가능)
- ~~#3 B검증~~ ✅ 07-06 통과 (Problems 발동 + Slack 미수신 + 서비스/SLA 무영향 — 판정·격리 설계 실증됨)
